...
class sttMod(loader.Module):
    ...
    strings = {'name': 'sttMod',
               ...
               'afk_reason': '<b>Я сейчас не принимаю голосовые сообщения • I do not accept voice messages right now\nПричина • Reason:</b> <i>{}</i>',
    ...
    async def watcher(self, message):
        ...
        if strict_mode is True:
            ...
            if message.mentioned or message.is_private is True:
                if message.voice:
                    await utils.answer(message, '<b>Я сейчас не принимаю голосовые сообщения • I do not accept voice messages right now</b>')
                    await message.delete()
                    return
            ...
            if message.voice:
                if afk_state is not True:
                    ...
                    await utils.answer(message,
                                      '<b>Я сейчас не принимаю голосовые сообщения • I do not accept voice messages right now</b>')
        if not state:
            return
